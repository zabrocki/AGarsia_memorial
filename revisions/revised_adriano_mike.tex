...
once properly understood, and his insight to do so.
...
In the Pisot case, conjugates inside the unit circle force systematic cancellations 
...
He is remembered for a legendary 
two-pager on the \textit{Hopf Maximal Ergodic Theorem},
replacing nearly fifty pages of buildup and proof in standard 
texts such as Dunford--Schwartz.  Adriano had an uncanny ability to ``rewrite'' 
mathematics, stripping away clutter until the logic of a theorem 
seemed not just clear, but inevitable. 
This talent brought him true joy: he not only advanced math by writing and rewriting it until it became fully comprehensible, 
but also shared this clarity with others. He published several books, including two coauthored with \"Omer E\u{g}ecio\u{g}lu in his nineties.



In the mid--1970s, Adriano’s research trajectory shifted when Alain Lascoux visited UCSD and 
sold him on the beauty of combinatorics. At the time, and to some extent still today, combinatorics was often looked 
down upon by mathematicians working in fields of ``higher stature,'' emphasizing abstraction and sweeping generality. 
...
Adriano’s influence in combinatorics extended well beyond identities and
involutions. He also made foundational contributions to the study of
Cohen--Macaulay posets and rings. He conjectured that all Cohen--Macaulay posets
are partitionable, reflecting a guiding principle that he would carry and exploit
for the remainder of his career:
strong algebraic conditions often manifest themselves in combinatorial structure. 
Garsia’s poset conjecture remains open and continues to shape current research, 
providing an early example of this powerful marriage between algebra and combinatorics.

By the end of the 1980s, Garsia was directing his explosive enthusiasm, high futility threshold, 
and passion for concrete structure fully to the field of algebraic combinatorics, which would 
remain his home until the end of his life. 
Here, he launched a vast program that still inspires combinatorialists,
representation theorists, and geometers today. The venture began when
Garsia invited Ian Macdonald to lecture at UCSD in 1988. There, Macdonald
...
$\mathbb Q(q,t)[x_1,x_2,\ldots]$. He showed that these polynomials form an
...
Garsia was immediately intrigued and had an insight that entirely changed the 
face of Macdonald's work, inspiring combinatorialists, algebraists, and
geometers for decades to come.  He built on work from the late 1900's
which used $S_n$-representations as a mechanism to produce symmetric functions.
In particular, Frobenius concretely realized the decomposition of an $S_n$-module $V$ 
into its irreducible components $V^\lambda$ through
the Schur-positive symmetric function arising via the Frobenius map,
\[
\mathcal F(V) = \sum_{\lambda}
(\text{multiplicity of } V^\lambda \text{ in } V)\, s_\lambda .
\]

Garsia proposed reversing this perspective: construct an underlying representation-theoretic 
object starting from a Schur positive symmetric function.
%He reformulated Macdonald’s positivity conjecture
%as one of Schur positivity by introducing the 
%{\it modified Macdonald polynomials},
%\[
%H_\mu(x;q,t) = \sum_{\lambda} K_{\lambda\mu}(q,t)\, s_\lambda ,
%\]
%wherein the conjectured positive coefficient arise as Schur expansion
%coefficients. He proposed that the conjectured
%($q$ and $t$) Schur-positivity of this symmetric function 
%could be explained by constructing an underlying
%representation-theoretic object. 
%The conjecture $K_{\lambda\mu}(q,t)\in \mathbb N[q,t]$ was thus recast as the problem
%of realizing these polynomials as Frobenius images of {\it bigraded} $S_n$-modules
%$D_\mu=\bigoplus_{r,d} D_\mu^{r,d}$:
%
%%
%
...
of ($q,t$)-Schur positivity, and he proposed to approach 
the conjecture $K_{\lambda\mu}(q,t)\in \mathbb N[q,t]$ 
by realizing these polynomials as the Frobenius images 
of some {\it bigraded} $S_n$-modules 
...
As a precursor, Garsia developed explored his approach on the $q=0$ case.
The $K_{\lambda\mu}(q,t)$ reduce to the \emph{Kostka-Foulkes polynomials}, a well-studied
family arising in contexts such as the cohomology of Springer fibers, % \cite{Spr,HoSp},
representation theory of  $GL_n(\mathbb{F}_q)$, %\cite{Green}, and
and affine Kazhdan-Lusztig theory. % \cite{Lus}.  
%Further, Lascoux-Sch\"utzenberger gave
%a beautiful combinatorial characterization in terms of Young tableaux.
Garsia and Procesi gave a new proof that $K_{\lambda\mu}(0,t)\in\mathbb N[t]$
by finding graded $S_n$-modules (the {\it Garsia-Procesi modules}) whose Frobenius characteristics are $\tilde H_\mu(x;0,t)$.
...
%interest.
Garsia's enthusiasm for the project infected Haiman, and together they aimed 
higher.
They defined bigraded $S_n$-modules $D_\mu$ given by the subspace of the polynomial ring
${\mathbb Q}[x_1,\ldots ,x_n;y_1,\ldots,y_n]$ spanned by
all derivatives of a certain simple determinant  $\Delta_\mu$.
Here, $S_n$ acts diagonally via
$(x_1,\ldots,x_n;y_1,\ldots,y_n)\mapsto
(x_{\sigma(1)},\ldots,x_{\sigma(n)};y_{\sigma(1)},\ldots,y_{\sigma(n)})$.
They conjectured that the dimension of $D_\mu$ is $n!$ and
that \eqref{e:FonD} is satisfied.
...
Their subsequent efforts quickly led them to a vastly bigger picture
given by the space where the determinants $\Delta_\mu$ are its minimal alternants.
This is the space of {\it diagonal harmonics},
$$
 \text{DH}_n=
 \big\{
...
$$
 ...
 \big\},
 $$
 a bigraded module under the diagonal $S_n$-action.

A prolific data gathering period ensued in the 90's.
Fueled by computer experimentation, Garsia and Haiman amassed identities 
that brought forth
%solidified a connection to 
the most familiar combinatorial structures  of the time.
%to parking functions was confirmed in the strongest possible manner.
For instance, the dimension of $\text{DH}_n$ is $(n+1)^{n-1}$,
counting parking functions on $n$ cars,
and the dimension of the submodule of  $\text{DH}_n$ affording the
sign representation is the $n$th Catalan number ${1\over n+1}{2n \choose n}$.
%and strongly suggested that the Frobenius characteristic
%of $\text{DH}_n$ could be given a purely
%combinatorial description.
% in terms of the most familiar combinatorial structures  of the time.
%The close study of $\mathcal F({\rm DH}_n,x;q,t)$ led Garsia
%and Haiman to a explicit formula for the Frobenius
%characteristic of $DH_n$ in terms of the ${\tilde H}_{\mu}$.

While they defined $D_\mu$ precisely so that modified Macdonald polynomials are $\mathcal F(D_\mu)$, 
the symmetric function $\mathcal F(\text{DH}_n)$ was a new mystery to be reckoned with.
%Garsia and Haiman conjectured
%\begin{equation}
%\label{DHinMac}
%\mathcal F(\text{DH}_n)=\sum _{\mu}  \tilde H_{\mu}(x;q,t) d_{\mu}(q,t)
%\end{equation}
%for certain explicit rational functions $d_{\mu}(q,t)$. % \cite{GaHa}.
%Soon after,  
It was in this pursuit that Garsia and Bergeron uncovered
the importance of an operator $\nabla$ that takes $\tilde H_\mu$ as its eigenfunctions.
...
symmetric function $\sum_{i_1<\cdots<i_n}x_{i_1}\cdots x_{i_n}$.
The $\nabla$ operator became a mainstay in the theory of Macdonald polynomials
and in fact was later discovered by be but one of a family of operators
deeply connected to the elliptic Hall algebra.

The pair developed {\it science fiction} around the theory of Macdonald polynomials
where $\nabla$ was featured prominently.


{\bf plethystic }

{\bf insert polynomiality result}



%Garsia and Bergeron introduced the
%operator $\nabla$, designed to take $\tilde H_\mu$ as
%its eigenfunctions, and proved that the right side
%of \eqref{DHinMac} can be expressed compactly as
%%$\nabla e_n$ where $e_n$ is the $n$th elementary
%symmetric function $\sum_{i_1<\cdots<i_n}x_{i_1}\cdots x_{i_n}$.
%The $\nabla$ operator plays an important role in many aspects of this theory.
%{\bf elliptic Hall algebra}

%During the extended period when \eqref{DHinMac} was
%a conjecture, 

%Haiman spent a decade putting together new algebro-geometric tools,
%culminating in proofs of these conjectures using the geometry of
%Hilbert schemes of points in $\mathbb C^2$.

A big breakthrough in the study of $\text{DH}_n$ was made
when Haiman proved \eqref{DHinMac}
using the interpretation of ${\tilde H}_{\mu}$
in terms of the Hilbert scheme  and attracted the attention
of algebraic geometers to the space of diagonal harmonics and
the modified Macdonald polynomials: 
%For later reference, we summarize Haiman's in symbols:
\begin{equation}
\label{e nabla DH}
\mathcal F(\text{DH}_n)=\nabla e_n.
%= \mathfrak p_{1}^{n+1/n}1,
\end{equation}
%where the  $\mathfrak p_{1}^{n+1/n}$ is the operator mentioned earlier.

{\bf talk about rational stuff}

%Haiman's theorem was exceedingly lucrative, stimulating
%many surprising directions of research.  Nonetheless,
%many of the conjectures that arose along the way still
%remain as major open problems.
%In fact, a fundamental piece of the picture is
%still missing; there is no explicit representation theoretic
%grasp on the diagonal harmonics module.
%Although \eqref{e nabla DH} gives an
%explicit formula for its Frobenius characteristic,
%it involves unmanageable rational functions of
%$q$ and $t$, and in no way exposes
%a decomposition into irreducible components.
%
A problem of central importance is to understand
the Schur expansion of \eqref{e nabla DH}.  Descriptions for
the coefficients $C_\lambda(q,t)\in\mathbb N[q,t]$ arising in
\begin{equation}
\label{DH2schur}
\mathcal F(\text{DH}_n) = \sum_\lambda C_\lambda(q,t) \, s_\lambda(x)
\end{equation}
have long been sought.  The earliest investigations of diagonal harmonics
by Garsia and Haiman gave hints to this end.  For example, it was
clear that $C_{1^n}(1,1)$ counted the number of Dyck paths
since the dimension of the submodule of  $\text{DH}_n$ affording the sign representation
is the $n$th Catalan number.
The natural question became one of finding a
pair of non-negative integers $(a(\pi),d(\pi))$ associated
in some way to a Dyck path $\pi$ so that
$$
C_{1^n}(q,t) = \sum_{\pi} q^{a(\pi)}\, t^{d(\pi)}\,.
$$

A formula in this spirit eluded researchers for many years
until Haglund brought forth such a candidate.  It was 
Garsia and Haglund together who established this formula.
To date, the remainder of this problem stands wide open.
An important step towards understanding the general case is the
{\it Shuffle conjecture}, which gives a combinatorial prediction for the coefficient of a
monomial in $\nabla e_n$, expressed in terms of simple $q,t$-weights
associated to parking functions.

Because of the relationship between the character of $\text{DH}_n$
and classical combinatorial functions,  the
study of $\nabla e_n$ and the combinatorics of $\text{DH}_n$
has been a central topic of
research in algebraic combinatorics over the last $20$ years.
There are several books on this topic, and it has been
the subject of over one hundred research articles.
Many of these articles involve the $\nabla$ operator and a variety of
extensions of the Shuffle conjecture.

\begin{wrapfigure}{r}{0.33\textwidth}
  \centering
  \includegraphics[width=\linewidth]{../images/mathtalkwadriano.jpg}
  \caption*{\footnotesize His passion for math was contagious.}
\end{wrapfigure}
Yet more than any achievement, Adriano lived to pass on his infectious enthusiasm 
for mathematics. Whether he was simplifying a 50-page proof in analysis or finding 
the hidden symmetry in a partition, he believed that the most profound truths were 
